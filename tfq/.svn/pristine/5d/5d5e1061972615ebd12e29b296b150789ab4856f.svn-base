...
class Tilemap 
{
	...
	public function new(_graphic:BitmapData, _mapWidth:Int, _mapHeight:Int, _tilesize:Int)
	{
		graphic = _graphic;
		mapWidth = _mapWidth;
	...
	public function render(_data:BitmapData, camera:Rectangle):Void
	{
		...
		while(i-->up)
		{
			...
			while(j-->left)
			{
				...
				_data.copyPixels(tileGraphics[index],tileGraphics[index].rect,tempPoint);
			}
		}
