...
class TileEntity implements Unit
{
	...
	public function solveTile(tx:Int, ty:Int, tileW:Int, tileH:Int, limits:Rectangle):CollisionData
	{
		...
		if (colx && coly)
		{
			...
			var last:Rectangle = new Rectangle(x - velX, y - velY, width, height);
			var tr:Rectangle = new Rectangle(tx, ty, tileW, tileH);
			var ur:Rectangle = last.union(tr);
			var disjointTime = Math.min(ur.width / Math.abs(velX), ur.height / Math.abs(velY));
			if (disjointTime < (1 - coltime))
			{
